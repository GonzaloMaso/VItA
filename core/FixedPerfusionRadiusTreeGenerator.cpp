...
FixedPerfusionRadiusTreeGenerator::FixedPerfusionRadiusTreeGenerator(
...
AbstractStructuredCCOTree *FixedPerfusionRadiusTreeGenerator::generate() {
...
int FixedPerfusionRadiusTreeGenerator::isValidRootSegment(point xNew,
		int iTry) {
...
int FixedPerfusionRadiusTreeGenerator::isValidSegment(point xNew, int iTry) {
...
AbstractDomain * FixedPerfusionRadiusTreeGenerator::getDomain() {
	return domain;
}

vector<AbstractStructuredCCOTree*> FixedPerfusionRadiusTreeGenerator::getTrees() {
	vector<AbstractStructuredCCOTree*> trees;
	trees.push_back(tree);
	return trees;
}

void FixedPerfusionRadiusTreeGenerator::enableConfigurationFile(
		string filename) {
	this->isGeneratingConfFile = 1;
	this->confFilename = filename;
}

void FixedPerfusionRadiusTreeGenerator::generatesConfigurationFile(ios::openmode mode) {
...
void FixedPerfusionRadiusTreeGenerator::markTimestampOnConfigurationFile(
		string label) {
	if (isGeneratingConfFile) {
...
void FixedPerfusionRadiusTreeGenerator::closeConfigurationFile() {
...
AbstractStructuredCCOTree* FixedPerfusionRadiusTreeGenerator::resume() {
...
void FixedPerfusionRadiusTreeGenerator::observableModified(IDomainObservable* observableInstance) {
