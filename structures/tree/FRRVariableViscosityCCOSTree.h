...
#ifndef FIXEDRADIUSROOTVARIABLEVISCOSITYCCOTREELEGACY_H_
#define FIXEDRADIUSROOTVARIABLEVISCOSITYCCOTREELEGACY_H_
 ...
 * testing. Current implementation is in OpenMP although vessel testing should be executing in MPI
 * and bifurcation testing in OpenMP for best results.
 * Specifications:
 * 	- Fixed domain;
 * 	- Given flow Q;
 ...
 */
class FRRVariableViscosityCCOSTree : public AbstractStructuredCCOTree {
...
public:
	 ...
	 */
	FRRVariableViscosityCCOSTree(point xi, double rootRadius, double qi, AbstractConstraintFunction<double,int> *gam, AbstractConstraintFunction<double,int> *epsLim, AbstractConstraintFunction<double,int> *nu, double minAngle, double refPressure, double resistanceVariationTolerance, GeneratorData *instanceData);

	/**
	 * Common destructor.
	 */
	~FRRVariableViscosityCCOSTree();
	 ...
	 */
	FRRVariableViscosityCCOSTree *clone();
	...
	/**
	 * Save in @p filename the current CCO tree in VTK format.
...
private:
	 ...
	 */
	void updateTree(vessel *root, FRRVariableViscosityCCOSTree *tree);
	/**
...
#endif /* FIXEDRADIUSROOTVARIABLEVISCOSITYCCOTREELEGACY_H_ */
